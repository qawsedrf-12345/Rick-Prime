--=============================================================
-- SISTEMA COMPLETO v7.1 - RESPAWN + MORPH + FREEZE 0.65s
--=============================================================
...
local function removerBlur(duracao)
    ...
    local tween = TweenService:Create(blurEffect, TweenInfo.new(duracao or 0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = 0 })
    tween:Play()
...
local function removerColorCorrection(duracao)
    ...
    local tween = TweenService:Create(colorCorrectionEffect, TweenInfo.new(duracao, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Saturation = 0, Contrast = 0 })
    tween:Play()
    tween.Completed:Connect(function()
        if colorCorrectionEffect then colorCorrectionEffect:Destroy(); colorCorrectionEffect = nil end
    end)
end
...
local function iniciarFreeze(character, duracao)
    ...
    -- 🔥 Trava a câmera (congela na posição atual)
    ...
    -- 🔥 Trava a posição do personagem (evita física empurrar)
    local connPos = RunService.Heartbeat:Connect(function()
        if freezeAtivo and rootPart and rootPart.Parent then
...
local function cameraShake(intensidade, duracao, rotacional)
    ...
    conn = RunService.RenderStepped:Connect(function()
        ...
        local progresso = 1 - ((tick() - inicio) / duracao)
...
local function aplicarMorph()
    ...
    if not modelSuccess or not generatedModel then return false end
...
local function efeitoMorte()
    if CONFIG.MorteFlashCiano then
        task.spawn(function()
            flashTela(CONFIG.MorteCorCiano, CONFIG.MorteFlashDuracao)
        end)
    end
    if CONFIG.MorteRGBSplit then
        task.spawn(function()
            efeitoRGBSplit(CONFIG.MorteRGBDuracao, CONFIG.MorteRGBDeslocamento, CONFIG.MorteRGBPulsos)
        end)
    end
    if CONFIG.MorteScanlines then
        task.spawn(function()
            efeitoScanlines(CONFIG.MorteScanlinesDuracao, CONFIG.MorteScanlinesVelocidade)
        end)
    end
    if CONFIG.MorteVinhetaVermelha then
        task.spawn(function()
            vinhetaTela(Color3.fromRGB(255, 0, 0), CONFIG.MorteVinhetaDuracao, CONFIG.MorteVinhetaIntensidade)
        end)
    end
    ...
    if CONFIG.MorteParticulas then
        task.spawn(function()
            ...
            emitter:Emit(CONFIG.MorteParticulasQuantidade)
    ...
    if CONFIG.MorteGlitchBlocks then
        task.spawn(function()
            efeitoGlitchBlocks(CONFIG.MorteGlitchDuracao, CONFIG.MorteGlitchQuantidade, CONFIG.MorteCorCiano)
        end)
    end
...
local function efeitoRespawn()
    ...
    if CONFIG.RespawnFragmentos then
        task.spawn(function()
            for i = 1, CONFIG.RespawnFragmentosQuantidade do
    ...
    if CONFIG.RespawnOndasChoque then
        task.spawn(function()
            efeitoOndas(CONFIG.RespawnOndasQuantidade, CONFIG.RespawnOndasDuracao, CONFIG.RespawnPortalCor)
        end)
    end
    ...
    if CONFIG.RespawnScanlines then
        task.spawn(function()
            efeitoScanlines(CONFIG.RespawnScanlinesDuracao, 2)
        end)
    end
...
RunService.Heartbeat:Connect(function()
    ...
    if agora - tempoUltimoSalvamento < 1 and ultimoCFrameSalvo 
...
player.CharacterAdded:Connect(function(character)
    ...
    -- 🥶 FREEZE IMEDIATO (antes de qualquer outra coisa)
    task.spawn(function()
